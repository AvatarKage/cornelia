...
async function parsePacks(packs) {
    root = {};
    currentNode = root;
    back = [];
    forward = [];
...
function createItem(key, v, pathOverride) {
    ...
    const typeSubText = file
        ? packText
        : `${count.folders > 0 ? `${count.folders} Folder${isPlural(count.folders)}` : ""}${count.folders > 0 && count.files > 0 ? ", " : ""}${count.files > 0 ? `${count.files} Preset${isPlural(count.files)}` : ""}`;

...
function addTextElement(svgDoc, x, y, r, fontSize, content, fill, anchor = "middle", iconMethod, baseColor, isCustomIconColor, iconColor) {
    ...
    if (iconMethod === "engrave") {
        if (!svgDoc.getElementById("innerShadow")) {
            ...
            filter.append(feOffset, feGaussianBlur, feComposite1, feFlood, feComposite2, feComposite3);
    ...
    if (iconMethod === "emboss") {
        if (!svgDoc.getElementById("outerShadow")) {
            ...
            const feFlood = svgDoc.createElementNS("http://www.w3.org/2000/svg", "feFlood");
            feFlood.setAttribute("flood-color", isCustomIconColor
            ...
            filter.append(feMorphology, feOffset, feGaussianBlur, feFlood, feComposite, feMerge);
            defs.appendChild(filter);
            svg.appendChild(defs);
        }
    }
    ...
    const isSvgString = typeof content === "string" && content.includes("<svg");
    if (typeof content === "string" && !isSvgString) {
        ...
        if (viewBox) {
            ...
            imported.setAttribute("transform", `translate(${-vbWidth * scale / 2}, ${-vbHeight * scale / 2})`);
        }
        else {

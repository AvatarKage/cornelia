...
async function getPreviewSVG(font: string): Promise<string | null> {
...
async function convertTextToPaths(
    ...
    if (!response.ok) {
        throw new Error(
            `Font loading failed: ${response.status} ${fontUrl}`
        );
    }
    ...
    function getPathsBounds(paths: any) {
        let x1 = Infinity;
        let y1 = Infinity;
        let x2 = -Infinity;
        let y2 = -Infinity;
    ...
    texts.forEach((text) => {
        ...
        const x = Number(
            text.getAttribute("x") || 0
        );

        const y = Number(
            text.getAttribute("y") || 0
        );

        const fontSize = Number(
            text.getAttribute("font-size") || 16
        );
        ...
        const measurePaths = buildGlyphPaths(
            value,
            0,
            0,
            fontSize
        ...
        const finalPaths = buildGlyphPaths(
            value,
            offsetX,
            offsetY,
            fontSize
...
function getPreviewSVGForImage(): string | null {
    const svg = (e.svg as HTMLElement).innerHTML.trim();
    if (!svg) {
        alert("No SVG available to download");
        return null;

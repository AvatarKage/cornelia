...
function getFontFormat(fontUrl: string) {
    if (fontUrl.includes(".woff2")) return "woff2";
    if (fontUrl.includes(".woff")) return "woff";
    if (fontUrl.includes(".otf")) return "opentype";
    if (fontUrl.includes(".ttf")) return "truetype";
}

function removeExistingFontStyle(svgDoc: Document) {
    const existing = svgDoc.querySelector("style[data-injected-font='true']");
    if (existing) existing.remove();
}

async function injectFont(svgDoc: Document, font: string): Promise<void> {
    let base64: any;

document.addEventListener('DOMContentLoaded', () => {
    ...
    function resolveHexColor(input, type = 'background') {
        ...
        if (isHex(input)) return input.toUpperCase();
    ...
    function tooltip_display(text, target, direction) {
        ...
        if (visible && current_target === target) return;
        ...
        timeout_show = setTimeout(() => {
            ...
            contentHtml += lines
                .map((line, index) => {
                    if (index === 0) return `<div>${line}</div>`;
                    subtextOffset += 9;
    ...
    document.body.addEventListener('mouseleave', e => {
        if (e.target.closest('[tooltip]')) tooltip_hide();
    }, true);

    document.addEventListener('mousemove', e => {
        if (!e.target.closest('[tooltip]') && !tooltip.contains(e.target)) tooltip_hide();
    });

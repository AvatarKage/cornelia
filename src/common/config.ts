...
import readJSON from "./helpers/src/readJSON.js";

const file = fileURLToPath(import.meta.url);
const dir =  resolve(dirname(file), "..", "..")

dotenv.config({
    path: resolve(dir, "config", "public.env"),
});
...
const config = {
    ...
    folders: {
        ...
        assets: resolve(dir, "src", "common", "assets"),
    ...
    colors: await readJSON("./config/data/colors.json") as any,

    rules: {
        ignore: await readJSON("./config/rules/ignore.json") as any,
        exact: await readJSON("./config/rules/exact.json") as any
    },
